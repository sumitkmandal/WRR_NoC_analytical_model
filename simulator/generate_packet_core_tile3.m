...
%     if (rnd < u(2)) % u(2): lambda
 %       valid = 1;                              % A packet will be generated in this cycle
        ...
        if (u(1) == 1)
            sink_array = tile_1_sink;
        end
        
        if (u(1) == 2)
            sink_array = tile_2_sink;
        end
        
        if (u(1) == 3)
            sink_array = tile_3_sink;
        end
        
        if (u(1) == 4)
            sink_array = tile_4_sink;
        end
        
        if (u(1) == 5)
            sink_array = tile_5_sink;
        end
        
        if (u(1) == 6)
            sink_array = tile_6_sink;
        end
