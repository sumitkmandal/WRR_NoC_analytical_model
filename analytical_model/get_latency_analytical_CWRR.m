function [avg_latency, tile_to_tile_latency, avg_latency_dummy_sim, tile_to_tile_latency_dummy_sim] = ...
    get_latency_analytical_CWRR(lambda_in, num_tiles, T_serv, weights, pb, debug_required, mode)


lambda_array = zeros(num_tiles, num_tiles);
avg_latency = 0;
tile_to_tile_latency = zeros(num_tiles, num_tiles);
avg_latency_dummy_sim = 0;
tile_to_tile_latency_dummy_sim = zeros(num_tiles, num_tiles);

for src_idx = 1:num_tiles
    for dest_idx = 1:num_tiles
        
        if (dest_idx == 8)
            lambda_array(src_idx, dest_idx) = lambda_in*7;
        else
